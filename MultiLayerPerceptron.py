...
class MLP(NeuralNet):
    def __init__(self, input_size, input_shape, indices_names, h_dims, num_classes, destination_folder,
        ...
        if self.is_pruning:
            self.hebb_layers = HebbLayersMLP(input_size, input_shape, indices_names, self.num_classes, h_dims,
                                             ...
                                             new_ns=[16 for _ in range(len(h_dims))],)
    ...
    def run(self, n_epochs, verbose=1, clip_grad=0, is_input_pruning=False, start_pruning=3, show_progress=20,
            is_balanced_relu=False, plot_progress=2, hist_epoch=20, all0=False, overall_mean=False,
            hr=5, ratio_replace=0.05):
        ...
        for _ in range(self.epoch, n_epochs):
            ...
            self.eval()
            with torch.no_grad():
                if self.epoch % hebb_round == 0 and self.epoch != 0 and \
                        self.epoch >= start_pruning and self.is_pruning:
                    print("Computing Hebbian layers...")
                    self.fcs, self.valid_bool = self.hebb_layers.compute_hebb(
                        total_loss, self.epoch, results_path=self.results_path,
                        fcs=self.fcs, verbose=3, to_replace=ratio_replace)
                    ...
                    if self.a_dim > 0:
                        involment_df = pd.concat((involment_df, pd.DataFrame(
                            hebb_input_values.detach().cpu().numpy()
                    ...
                    print(involment_df.sort_values(by=[last_col], ascending=False),
                          file=file_involvment)
                ...
                for x, y in self.valid_loader:
                    ...
                    logits = self(x, valid_bool=self.valid_bool,
                                  input_pruning=is_input_pruning,
                                  start_pruning=start_pruning,
                                  is_balanced_relu=is_balanced_relu,
                                  all0=all0, overall_mean=overall_mean)
                    try:
                        targets = torch.max(y, 1)[1].long()
                    except:
            ...
            if self.epoch % plot_progress == 0:
                ...
                accuracies_histories = {"train": self.train_accuracy_history,
                                        "valid": self.valid_accuracy_history}
                labels = {"train": self.labels_train, "valid": self.labels_test}
                if self.epoch % show_progress == 0 and self.epoch % hebb_round == 0 \
                        and self.epoch != 0:
        ...
        mean_total_losses_histories = {"train": np.mean(
            np.array(self.train_total_loss_histories), axis=0),
            "valid": np.mean(np.array(self.valid_total_loss_histories), axis=0)}
        var_losses_histories = {"train": np.std(
            np.array(self.train_total_loss_histories), axis=0),
            "valid": np.std(np.array(self.valid_total_loss_histories), axis=0)}
        mean_accuracies_histories = {"train": np.mean(
            np.array(self.train_accuracy_histories), axis=0),
            "valid": np.mean(np.array(self.valid_accuracy_histories), axis=0)}
        var_accuracies_histories = {"train": np.std(
            np.array(self.train_accuracy_histories), axis=0),
