...
from utils.utils import create_missing_folders, glorot_init
...
class HebbLayersMLP(NeuralNet):
    def __init__(self, input_size, input_shape, indices_names, num_classes, Ns, hebb_rates, gt, hebb_rates_neurites, hebb_rates_multiplier,
                 new_ns, destination_folder, kernels=None, gt_neurites=None, lambd=1., clamp_max=1000000, clamp_min=-1000000, gt_input=-1000,
        ...
        self.init_function = glorot_init
        ...
        with torch.no_grad():
            ...
            try:
                ...
                if torch.cuda.is_available():
                    ...
                    fcs[-1].weight = nn.Parameter(Variable(torch.from_numpy(w3[:, indices_alive_neurons_out])))
                    fcs[-1].weight.grad = nn.Parameter(Variable(torch.from_numpy(wg3)))
                ...
                if torch.cuda.is_available():
                    ...
                    fcs[-1].weight = nn.Parameter(Variable(torch.from_numpy(w3)))
                    fcs[-1].weight.grad = nn.Parameter(Variable(torch.from_numpy(wg3)))
